...
class FindInAll(QThread):
    ...
    Instance variables:
      - undo_stack, QUndoStack: place there all what is done as a compound undo 
        event.
      - stop, bool: if true, the algorithm will stop at the next point
      - data_manager, TrackingData: object handling the current data
      - list_images, list of str: list of images where the points will be 
        searched
      ...
      - num_images, int: total number of images to look into (just used to 
        setup a correct percentage)
        ...
        try:
            ...
            for currentImage, image_name in enumerate(self.list_images[1:]):
                ...
                for currentPoint, id in enumerate(pts):
                    ...
                    new_pos, value = findTemplate(im_source, (pos.x(), pos.y()), template_size, (npos.x(), npos.y()), search_size, im_target)
                    if value >= 0.5:
                        new_pts.append(id)
...
def findTemplate(origin, template_pos, template_size, search_pos, search_size, target):
    ...
    Arguments:
      - origin, ndarray: image where the template is extracted (the image is 
        accessed as a matrix, i.e. the points (x,y) is found at origin[y,x])
      ...
      - target, ndarray: image where the template is searched (the image is 
        accessed as a matrix, i.e. the points (x,y) is found at target[y,x])
    """
...
def alignImages(data, alignment_data, translation, rotation):
    ...
    Arguments:
      ...
      - rotation, (str,int,int): type of rotation. The first value is the 
        rotation type. For now, it has to be "TwoPoint". The two other values 
        are the points id used for the rotation.
    """
    ...
    if translation == "Bounding-box centre":
        ...
        for i,d in enumerate(alignment_data):
            ...
            if not translation in d:
                raise AlgoException("Error, point %d is not in image %s. Cannot use it to align images." % (translation, d.image_name))
