...
def cellsToWatch(data, pts_id):
    ...
    watching_cells = {}
    for pt_id in pts_id:
        watching_cells.update((cid, (cells[cid], cells_lifespan[cid]))
                              for cid in data.cell_points[pt_id])
...
class RemovePoints(PointsCommand):
    def __init__(self, data_manager, image_name, pts_id, cmd_id=-1, parent=None):
        PointsCommand.__init__(self, "Remove point(s) from image %s" % image_name,
                               data_manager, image_name, cmd_id, parent)
        data = self.current_data
...
class RemovePointsInImages(PointsCommand):
    def __init__(self, title, data_manager, image_name, pts_id, image_list, cmd_id=-1, parent=None):
        ...
        self.images = [[im
                        ...
                        if pt in data_manager[im]] for pt in pts_id]
...
class RemovePointsInAllImages(RemovePointsInImages):
    def __init__(self, data_manager, image_name, pts_id, cmd_id=-1, parent=None):
        ...
        RemovePointsInImages.__init__(self, "Remove point(s) in all images",
                                      data_manager, image_name, pts_id, images, cmd_id, parent)
...
class RemovePointsFromImage(RemovePointsInImages):
    def __init__(self, data_manager, image_name, pts_id, cmd_id=-1, parent=None):
        ...
        RemovePointsInImages.__init__(self, "Remove point(s) starting at image %s" % image_name,
                                      data_manager, image_name, pts_id, images, cmd_id, parent)
...
class RemovePointsToImage(RemovePointsInImages):
    def __init__(self, data_manager, image_name, pts_id, cmd_id=-1, parent=None):
        ...
        RemovePointsInImages.__init__(self, "Remove point(s) up to image %s" % image_name,
                                      data_manager, image_name, pts_id, images, cmd_id, parent)


class AddPoints(PointsCommand):
    def __init__(self, data_manager, image_name, ids, pos, cmd_id=-1, parent=None):
        PointsCommand.__init__(self, "Add point(s) to image %s" % image_name,
                               data_manager, image_name, cmd_id, parent)
        data = self.data_manager
        if ids is None:
            dpos = {}
...
class SplitCells(PointsCommand):
    def __init__(self, data_manager, image_name, cell_id, cmd_id=-1, parent=None):
        PointsCommand.__init__(self, "Split cell %s at image %s" % (cell_id, image_name),
                               data_manager, image_name, cmd_id, parent)
...
class MergeCells(PointsCommand):
    def __init__(self, data_manager, image_name, cell_id, new_cell_id, cmd_id=-1, parent=None):
        PointsCommand.__init__(self, "Merge cell %s with cell %s" % (cell_id, new_cell_id),
                               data_manager, image_name, cmd_id, parent)
...
class SplitPointsId(PointsCommand):
    def __init__(self, data_manager, image_name, pts_id, cmd_id=-1, parent=None):
        ...
        PointsCommand.__init__(self, "Split point %s at image %s" % (pts_id[0], image_name),
                               data_manager, image_name, cmd_id, parent)
        pt_id = pts_id[0]
        self.pt_id = pt_id
...
class ChangePointsId(PointsCommand):
    def __init__(self, data_manager, image_name, pts_id, new_pts_id, cmd_id=-1, parent=None):
        ...
        PointsCommand.__init__(self, "Merge point %s with point %s" % (pts_id[0], new_pts_id[0]),
                               data_manager, image_name, cmd_id, parent)
        pt_id = pts_id[0]
...
class InsertPointInWallCommand(TrackingCommand):
    def __init__(self, data_manager, new_pt, wid, cmd_id=-1, parent=None):
        TrackingCommand.__init__(self, "Insert point %d in wall (%d,%d)" % ((new_pt,)+wid),
                                 cmd_id, parent)
        ...
        if self.first_run:
            ...
            if saved_cells:
                ...
                for cid, old_pt_ids in zip(changed_cells, saved_cells):
                    ...
                    if left_pts:
                        actions.append("On cell %d, removed duplicated points: %s" %
                                       (cid, ",".join(str(ptid) for ptid in left_pts)))
